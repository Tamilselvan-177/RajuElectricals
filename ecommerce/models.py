...
class Product(models.Model):
    ...
    vendor = models.CharField(max_length=150, null=False, blank=False, default="Raju Electricals and Hardwares")
    ...
    a_tag = models.TextField(max_length=100,null=True)
    pdf_manual = models.FileField(upload_to='product_pdfs/', null=True, blank=True, help_text="Upload a PDF manual")
...
class Banner(models.Model):
    product_image = models.ImageField(upload_to=getFileName, null=True, blank=True)
    bannerAbout = models.ImageField(upload_to=getFileName, null=True, blank=True)
    from django.db import models

# Utility function for file uploads
def getFileName_new(instance, filename):
    return f"slider_images/{filename}"

# Image model to hold individual images
class SliderImage(models.Model):
    images = models.ManyToManyField('Image', blank=True, related_name='slider_images')

    def __str__(self):
        return f"Slider with {self.images.count()} images"

class Image(models.Model):
    image = models.ImageField(upload_to=getFileName_new)

    def __str__(self):
        return self.image.name
class Banner(models.Model):
    # Single image for the bannerAbout field
    bannerAbout = models.ImageField(upload_to=getFileName, null=True, blank=True)
